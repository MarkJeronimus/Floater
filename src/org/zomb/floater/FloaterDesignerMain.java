...
public class FloaterDesignerMain extends JFrame
		implements WindowListener, MouseInputListener, ActionListener, DropTargetListener {
	...
	private void drawImage() {
		...
		imagePanel.g.translate(-0.5, -0.5);
		imagePanel.g.drawImage(vm.img,
		                       0, 0, (int)(width * zoom) + 1, (int)(height * zoom) + 1,
		                       0, 0, width, height,
		                       null);
		imagePanel.g.translate(0.5, 0.5);
	}
	...
	private void updateStackTable() {
		...
		for (int i = 0; i <= vm.sp; i++) {
			...
			address.append(i + FloaterConstants.ADDRESS_START);
			if (i < vm.sp - 1) {
				address.append(" / ").append(i - vm.sp + 1);
			}
